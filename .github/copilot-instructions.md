...
- **`agents/`** — Agent catalog, Mastra runtime, Claude SDK loader, tool implementations, orchestration pipelines. Published as `@techtide/agents`. The 61-agent hierarchy (1 CEO → 10 orchestrators → 50 workers, five per orchestrator) is defined statically in `agents/src/core/registry.ts`, not in the database. Agent IDs follow `{tier}-{name}` convention (`ceo-chief`, `orch-strategy`, `worker-research`). Orchestration module (`agents/src/orchestration/`) provides composable pipeline primitives: `chain`, `parallel`, `route`, `evalLoop`.
