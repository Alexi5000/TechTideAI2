...
export function Sidebar({ collapsed, onToggleCollapse }: SidebarProps) {
  return (
    <aside
      className={cn(
        "fixed left-0 top-0 z-[var(--z-sticky)] h-screen bg-[var(--surface-2)] border-r border-[var(--stroke)]",
    ...
    >
      ...
      <div className="h-[var(--header-height)] flex items-center px-4 border-b border-[var(--stroke)]">
        <Link to="/dashboard" className="flex items-center gap-3 overflow-hidden">
          <div className="h-9 w-9 rounded-xl bg-[var(--accent)] flex-shrink-0" />
          <span
            className={cn(
              "font-semibold text-[var(--ink)] whitespace-nowrap transition-opacity duration-[var(--duration-fast)]",
      ...
      <div className="p-3 border-t border-[var(--stroke)]">
        <button
          ...
          className={cn(
            ...
            "text-[var(--muted-strong)] hover:bg-[var(--surface-1)] transition-colors",
            collapsed && "px-0",
          )}
