...
describe("EvalHarness", () => {
  it("runs the suite end-to-end against a stub runtime", async () => {
    ...
    const runtime = makeRuntime({
      "orch-ava": { success: true, output: { ok: true }, events: [] },
      "orch-audit": { success: true, output: { ok: false }, events: [] },
    });
    ...
    // t3's agent ("orch-cipher") is registered but the runtime stub returns
